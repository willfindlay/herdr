...
`AgentInfo` also exposes an optional `git_status` object with `ahead`, `behind`, and `dirty` counts for the agent pane's checkout, so two agents in different linked worktrees of one repository report their own counts. `ahead` and `behind` compare the branch with its upstream and are omitted without one. `dirty` is the number of entries `git status` lists. The object is omitted while no refresh has run or the pane sits outside a repository, and that refresh runs only while `ui.sidebar.agents.git_status.enabled` is set and the sidebar rows that agent resolves (its `rows_by_agent` override, or the default `rows` otherwise) include the `git_status` token.
