...
- `git_status`: Git ahead, behind, and dirty counts (the entries `git status` lists) for the agent's checkout, nonzero parts only, drawn at the right edge of its row once `ui.sidebar.agents.git_status.enabled` is set.
...
The `git_status` agent token is off by default because it runs `git status` in the background for every agent checkout. Turn it on with:
...
Tokens render in their configured order, except `git_status` on an agent row, which sits at the right edge when it fits and is skipped otherwise. Herdr normally separates adjacent values with ` · ` and uses a single space after `state_icon`. Missing values and their separators disappear; a row disappears when none of its tokens have a value. Each layout may contain at most 16 rows, with at most 16 tokens in each row.
